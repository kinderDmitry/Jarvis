...
/** JARVIS premium assistant UI. Fixed geometry, responsive spacing and centered controls. */
public class MainActivity extends Activity {
    private static final int BG=Color.rgb(2,6,11), PANEL=Color.rgb(5,15,25), PANEL2=Color.rgb(7,20,32);
    private static final int CYAN=Color.rgb(82,220,255), BORDER=Color.rgb(22,91,139), WHITE=Color.rgb(242,247,255), MUTED=Color.rgb(126,155,181), GREEN=Color.rgb(72,231,177), RED=Color.rgb(255,103,119);
    ...
    private TextView action(String label,View.OnClickListener listener){
        TextView v=text(label,13);v.setGravity(Gravity.CENTER);v.setTypeface(Typeface.DEFAULT,Typeface.BOLD);v.setSingleLine(false);v.setMaxLines(2);v.setEllipsize(null);
        v.setPadding(dp(10),dp(2),dp(10),dp(2));v.setMinHeight(dp(52));v.setBackground(surface(BORDER,0xF2071622,18));v.setOnClickListener(listener);
        return v;
    }
    private TextView icon(String glyph,View.OnClickListener listener){TextView v=action(glyph,listener);v.setTextColor(CYAN);v.setTextSize(android.util.TypedValue.COMPLEX_UNIT_SP,23);v.setGravity(Gravity.CENTER);v.setPadding(0,0,0,0);return v;}
    ...
    private void handleIntent(Intent i){
        ...
        if(q!=null&&!q.trim().isEmpty())main.postDelayed(()->command(q),120);
    ...
    private void buildUi(){
        ...
        LinearLayout page=new LinearLayout(this);page.setOrientation(LinearLayout.VERTICAL);page.setPadding(dp(20),dp(16),dp(20),dp(190));
        scroll.addView(page,new ScrollView.LayoutParams(-1,-1));root.addView(scroll,new FrameLayout.LayoutParams(-1,-1));

        LinearLayout header=new LinearLayout(this);header.setGravity(Gravity.CENTER_VERTICAL);
        header.addView(icon("☰",v->showTools()),new LinearLayout.LayoutParams(dp(56),dp(56)));
        ...
        header.addView(brand,new LinearLayout.LayoutParams(0,dp(56),1));header.addView(icon("⚙",v->openSettings()),new LinearLayout.LayoutParams(dp(56),dp(56)));page.addView(header,new LinearLayout.LayoutParams(-1,dp(56)));
        ...
        network=text("●  В СЕТИ",11);network.setGravity(Gravity.RIGHT|Gravity.CENTER_VERTICAL);network.setTypeface(Typeface.DEFAULT,Typeface.BOLD);statusRow.addView(network,new LinearLayout.LayoutParams(0,dp(26),1));page.addView(statusRow,margin(-1,26,0,6,0,0));updateNetwork();

        core=new JarvisCoreView(this);core.setState("ГОТОВ");core.startPulse();core.setOnClickListener(v->listen());
        page.addView(core,margin(-1,230,0,4,0,0));
        ...
        replyBody=text("Нажмите на ядро или скажите «Привет, Джарвис»",16);replyBody.setTypeface(Typeface.DEFAULT,Typeface.BOLD);replyBody.setMaxLines(3);replyBody.setEllipsize(TextUtils.TruncateAt.END);response.addView(replyBody,margin(-1,56,0,6,0,0));page.addView(response,margin(-1,94,0,8,0,0));

        LinearLayout voiceRow=new LinearLayout(this);voiceRow.setGravity(Gravity.CENTER);TextView speak=action("◉  ГОВОРИТЬ",v->listen());voiceRow.addView(speak,new LinearLayout.LayoutParams(0,dp(52),1));
        TextView stop=action("■  СТОП",v->stopAll());voiceRow.addView(stop,margin(0,52,8,0,0,0));page.addView(voiceRow,margin(-1,52,0,5,0,0));
        ...
        setContentView(root);buildBottom(root);
    }
    private LinearLayout panel(){LinearLayout p=new LinearLayout(this);p.setOrientation(LinearLayout.VERTICAL);p.setPadding(dp(18),dp(13),dp(18),dp(10));p.setBackground(surface(BORDER,0xF006131F,24));return p;}
    private void buildBottom(FrameLayout root){
        ...
        input=new EditText(this);input.setSingleLine(true);input.setTextColor(WHITE);input.setHintTextColor(MUTED);input.setTextSize(android.util.TypedValue.COMPLEX_UNIT_SP,17);input.setHint("Спросить JARVIS…");input.setGravity(Gravity.CENTER_VERTICAL);input.setPadding(dp(16),0,dp(12),0);input.setInputType(InputType.TYPE_CLASS_TEXT|InputType.TYPE_TEXT_FLAG_CAP_SENTENCES);input.setImeOptions(EditorInfo.IME_ACTION_SEND);input.setBackground(surface(BORDER,0xEE071724,20));input.setOnEditorActionListener((v,id,e)->{if(id==EditorInfo.IME_ACTION_SEND){sendText();return true;}return false;});composer.addView(input,new LinearLayout.LayoutParams(0,dp(72),1));
        TextView send=icon("➤",v->sendText());composer.addView(send,margin(72,72,8,0,0,0));FrameLayout.LayoutParams cp=new FrameLayout.LayoutParams(-1,dp(94),Gravity.BOTTOM);cp.setMargins(dp(12),0,dp(12),dp(88));root.addView(composer,cp);
        ...
        FrameLayout.LayoutParams np=new FrameLayout.LayoutParams(-1,dp(78),Gravity.BOTTOM);np.setMargins(dp(12),0,dp(12),dp(8));root.addView(nav,np);
    }
    private void addNav(LinearLayout bar,String glyph,String label,boolean active,View.OnClickListener click){
        ...
        TextView i=text(glyph,17);i.setGravity(Gravity.CENTER);i.setTextColor(active?CYAN:MUTED);item.addView(i,new LinearLayout.LayoutParams(-1,dp(31)));
        TextView t=text(label,9);t.setGravity(Gravity.CENTER);t.setSingleLine(true);t.setTypeface(Typeface.DEFAULT,Typeface.BOLD);t.setTextColor(active?WHITE:MUTED);item.addView(t,new LinearLayout.LayoutParams(-1,dp(18)));bar.addView(item,new LinearLayout.LayoutParams(0,dp(55),1));
    }
    private void showTools(){
        final Dialog d=new Dialog(this);LinearLayout box=new LinearLayout(this);box.setOrientation(LinearLayout.VERTICAL);box.setPadding(dp(18),dp(18),dp(18),dp(18));box.setBackground(surface(BORDER,0xFF06131F,26));
        TextView h=text("ИНСТРУМЕНТЫ JARVIS",18);h.setTextColor(CYAN);h.setTypeface(Typeface.DEFAULT,Typeface.BOLD);h.setGravity(Gravity.CENTER);box.addView(h,new LinearLayout.LayoutParams(-1,dp(44)));
        ...
        for(int i=0;i<labels.length;i++){final int k=i;box.addView(action(labels[i],v->{d.dismiss();if(k<6)command(cmds[k]);}),margin(-1,54,0,7,0,0));}
