...
public class MainActivity extends Activity {
    ...
    private void initTts(){tts=new TextToSpeech(this,r->{if(r!=TextToSpeech.SUCCESS)return;try{Locale ru=new Locale("ru","RU");tts.setLanguage(ru);Voice best=null;for(Voice v:tts.getVoices()){if(!"ru".equalsIgnoreCase(v.getLocale().getLanguage()))continue;if(v.getQuality()>=TextToSpeech.VOICE_QUALITY_NORMAL&&(best==null||v.getQuality()>best.getQuality()||(v.getQuality()==best.getQuality()&&v.isNetworkConnectionRequired()&&!best.isNetworkConnectionRequired())))best=v;}if(best!=null)tts.setVoice(best);tts.setSpeechRate(.92f);tts.setPitch(.94f);tts.setOnUtteranceProgressListener(new UtteranceProgressListener(){public void onStart(String id){speaking=true;}public void onDone(String id){speaking=false;setState("ГОТОВ");}public void onError(String id){speaking=false;setState("ГОТОВ");}});}catch(Throwable ignored){}});}
