...
public class AlarmReceiver extends BroadcastReceiver {
 @Override public void onReceive(Context c,Intent i){
  ...
  boolean timer="JARVIS_TIMER".equals(action); boolean interval="JARVIS_INTERVAL".equals(action); String text=timer?"Сэр, время таймера истекло.":interval?c.getSharedPreferences("jarvis_interval_timer",Context.MODE_PRIVATE).getString("label","Интервальный сигнал")+".":"Сэр, время будильника.";
  ...
  nm.notify(interval?79:(timer?78:77),b.build());
  if(timer)c.getSharedPreferences("jarvis_timer",Context.MODE_PRIVATE).edit().clear().apply();
  if(interval)JarvisEngine.IntervalTimerTool.rearm(c);
 }
}
